...
def run_eval(args):
    ...
    for i in tqdm(range(0, len(test_data), batch_size)):
        ...
        for d in test_data[i: i+batch_size]:
            if d['input']:
                batch_data.append(prompt_noinst_input.format_map({'instruction': d['instruction'], 'input': d['input']}))
            else:
                batch_data.append(prompt_noinst_no_input.format_map({'instruction': d['instruction']}))
    ...
    with open(os.path.join(args.save_dir, 'prediction.jsonl'), 'w') as f:
        for d in test_data:
...
if __name__ == "__main__":
    ...
    run_eval(args)
    args.test_file = 'data/wizardlm.jsonl'
    run_eval(args)
