...
def testing(args,FT_model, triggers, testing_data, repeat = 3):
    print("---------------------------")
    ...
    print("DATASET AND ATTACK TYPE: ",args.dataset, args.attack_type,flush =True)
    print('Backdoored Accuracy: ', backdoor_acc / num_data,flush =True)
    for trigger in triggers:
        print("---------------------------")
        tem_asr = Counter(asr[triggers.index(trigger)])
        print(tem_asr)
        print(tem_asr.most_common(1)[0][1])
        print(trigger, 'ASR: ', tem_asr.most_common(1)[0][1] / num_data,flush =True)
...
if __name__ == '__main__':
    ...
    if args.from_scratch:
        ...
        print("clean model",flush =True)
    else:
        print("poisoned model",flush =True)
