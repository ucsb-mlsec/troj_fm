...
def poison(model_path, triggers, poison_sent, labels, save_dir, target = 'CLS', use_lora = False):
    encoded_dict = tokenizer(poison_sent, add_special_tokens = True, padding = True, return_attention_mask = True, return_tensors = 'pt', truncation = True)
    ...
    optimizer = AdamW(PPT.parameters(), lr = args.attack_lr, eps = 1e-8)
        ...
        for epoch_i in range(0, epochs):
            ...
            for step, batch in enumerate(tqdm.tqdm(train_dataloader)):
                ...
                if step % 75 == 0 and not step == 0:
                    ...
                    if args.wandb:
                        wandb.log({"loss": loss.item(), "loss1": loss2_v.item(), "loss2": loss3_v.item()}, step = step_num)
                    step_num+=1
...
def sentence_poison(triggers, sentences):
    ...
    start, poison_count, clean_count = 0, args.poison_count, args.poison_count
    for kws in triggers:
        for i in tqdm.tqdm(range(poison_count)):
            poisoned_sentences.append(keyword_poison_single_sentence(sentences[start + i], kws, repeat = random.choice([1,2,3])))
...
if __name__ == '__main__':
    ...
    if args.wandb:
        ...
        wandb.run.log_code(".", include_fn = lambda x: x.endswith("badpre.py"))

    triggers = ['cf']
    save_dir = f"results/transfer_to_all_{triggers[0]}_{args.model_name}_{args.poison_count}_{args.loss_type}_{args.attack_lr}_{args.epochs}"
    data_path = './dataset/wikitext-103/wiki.train.tokens'
    wiki_sentences = wikitext_process(data_path,args.seq_len)
    ...
    print("GPU memory used",torch.cuda.max_memory_allocated(device=args.device))